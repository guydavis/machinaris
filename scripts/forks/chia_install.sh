...
if [ -z ${CHIA_BRANCH} ]; then
    ...
    if [[ "${arch_name}" == "x86_64" ]]; then
        curl -sLJO https://github.com/Chia-Network/chia-blockchain/releases/download/2.7.3/chia-blockchain-cli_2.7.3-1_amd64.deb
        apt-get install ./chia-blockchain-cli*.deb
    else
        curl -sLJO https://github.com/Chia-Network/chia-blockchain/releases/download/2.7.3/chia-blockchain-cli_2.7.3-1_arm64.deb
        apt-get install ./chia-blockchain-cli*.deb
    fi
