...
if [[ "${blockchain_db_download}" == 'true' ]] \
  ...
  echo "Sorry, Ecostake does not offer a recent blockchain DB for download via script.  Standard sync will happen over a few days..."
  ...
  if [[ -z ${farmer_address} || -z ${farmer_port} ]]; then
    ...
    if [ ! -f /root/.ecostake/farmer_ca/private_ca.crt ]; then
      ...
      if [ $response == '200' ]; then
        ...
        echo "Certificates response of ${response} from http://${farmer_address}:8942/certificates/?type=ecostake.  Try clicking 'New Worker' button on 'Workers' page first."
      fi
      rm -f /tmp/certs.zip 
    fi
