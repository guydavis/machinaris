...
if [[ "${blockchain_db_download}" == 'true' ]] \
  ...
  echo "Sorry, Petroleum does not offer a recent blockchain DB for download via script.  Standard sync will happen over a few weeks."
...
petroleum init --fix-ssl-permissions 2>&1  >/dev/null 
...
if [[ ${mode} =~ ^fullnode.* ]]; then
  ...
  if [ -f /root/.chia/machinaris/config/wallet_settings.json ]; then
    petroleum start farmer-no-wallet
  else
  ...
  if [[ -z ${farmer_address} || -z ${farmer_port} ]]; then
    ...
    petroleum configure --set-farmer-peer ${farmer_address}:${farmer_port}  2>&1 >> /root/.petroleum/mainnet/log/init.log
    petroleum configure --enable-upnp false  2>&1 >> /root/.petroleum/mainnet/log/init.log
