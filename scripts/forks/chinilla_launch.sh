...
if [[ "${blockchain_db_download}" == 'true' ]] \
  ...
  echo "Downloading Chinilla blockchain DB (many GBs in size) on first launch..."
  echo "Please be patient as takes minutes now, but saves days of syncing time later."
  /usr/bin/bash /machinaris/scripts/megacmd_setup.sh > /tmp/megacmd_setup.log 2>&1
  mkdir -p /root/.chinilla/mainnet/db/ && cd /root/.chinilla/mainnet/db/
  # Mega links for chinilla blockchain DB from their discord August 2022
