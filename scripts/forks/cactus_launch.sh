...
if [[ ${mode} == 'fullnode' ]]; then
  ...
  if [[ -z ${farmer_address} || -z ${farmer_port} ]]; then
    ...
    if [ ! -f /root/.cactus/farmer_ca/private_ca.crt ]; then
      ...
      response=$(curl --write-out '%{http_code}' --silent http://${controller_host}:8928/certificates/?type=cactus --output /tmp/certs.zip)
      if [ $response == '200' ]; then
        unzip /tmp/certs.zip -d /root/.cactus/farmer_ca
      else
        echo "Certificates response of ${response} from http://${controller_host}:8928/certificates/?type=cactus.  Try clicking 'New Worker' button on 'Workers' page first."
      fi
      rm -f /tmp/certs.zip 
    fi
    if [ -f /root/.cactus/farmer_ca/private_ca.crt ]; then
