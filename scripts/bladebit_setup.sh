...
if [[ (${mode} == 'fullnode' || ${mode} =~ "plotter") && (${blockchains} == 'chia') ]]; then
    if [ ! -f /usr/bin/bladebit ] && [[ "${bladebit_skip_build}" != 'true' ]]; then
        ...
        if [[ "${arch_name}" = "x86_64" ]] || [[ "${arch_name}" = "arm64" ]]; then
            ...
            cd /bladebit/build
            curl -sLJO https://download.chia.net/bladebit/bladebit-cuda-v3.0.0-alpha2-ubuntu-x86-64.tar.gz.zip
            unzip bladebit-cuda-v3.0.0-alpha2-ubuntu-x86-64.tar.gz.zip
            tar -xzf bladebit-cuda-v3.0.0-alpha2-ubuntu-x86-64.tar.gz
