...
def load_current_disk_usage(disk_type, hostname=None):
    ...
    for host in worker.load_workers():
        ...
        if disk_type == 'plots':
            ...
            if created_at_max and datetime.datetime.strptime(created_at_max.created_at, '%Y%m%d%H%M') <= (datetime.datetime.now() - datetime.timedelta(minutes=STALE_MINUTES_DISK_STATS)):
