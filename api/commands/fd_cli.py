...
def reward_recovery(wallet_id, launcher_id, pool_contract_address):
    ...
    fd_env.update(vars)
    cmd = f"fd-cli nft-recover -l {launcher_id} -p {pool_contract_address} -nh 127.0.0.1 -np {rpc_port} -ct {network_path}/config/ssl/full_node/private_full_node.crt -ck {network_path}/config/ssl/full_node/private_full_node.key"
